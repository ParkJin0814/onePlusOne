...
public class SearchKeywordAspect {
    ...
    // 정상종료되었을때만 실행
    @AfterReturning(
            pointcut = "execution(* com.example.oneplusone.domain.product.controller.ProductController.productsPageV2(..))"
    )
        ...
        if (args.length >= 1 && args[0] instanceof String searchKeyword) {
            if (!searchKeyword.isBlank()) {
                ...
                if (auth != null && auth.getPrincipal() instanceof UserDetailsImpl userDetails) {
                    ...
                    searchService.popularSearch(searchKeyword, userId);
                    log.info("검색 키워드 저장: {}, userId: {}", searchKeyword, userId);
                }
            }
        }
    }
