...
### 적용 기능
1. **Redis Cache** 
   - 사용 이유 : 캐시 데이터 등록 및 삭제 적용
   - 캐시 등록 : `@Cacheable`
     - 적용 API : 상품 단건 조회, 상품 전체 조회, 주문 목록 조회
   - 캐시 삭제 : `@CacheEvict`
     - 적용 API
       - 상품 등록 : 캐시 삭제 - 상품 단건 조회
       - 상품 주문 : 캐시 삭제 - 상품 단건 조회, 상품 전체 조회, 주문 목록 조회 
       - 상품 삭제/수정 : 캐시 삭제 - 상품 단건 조회, 상품 전체 조회

2. **nGrinder**
   - 사용 이유 : 캐싱 적용 전/후 성능비교 테스트 
   - 적용 API : 상품 전체 조회

3. **Lock**
   - 사용 이유 : 동시 다발적 요청을 성공적으로 처리
   - 적용 API : 상품 주문
...
50명의 사용자가 1분동안 상품 전체 조회를 진행

캐시 적용시, 평균 테스트 시간은 **약 3.6배 감소**했으며, 처리된 작업은 **약 3.5배 증가**

| 항목         | 캐싱 미적용 (v1)  | Redis 캐싱 적용 (v2)         |
|------------| ------------ | ------------------------ |
| 평균 테스트 시간  | 103.49 ms        | **28.62 ms**                 |
| 최대 처리 요청 수 |  27,055 | **96,558**        |
...
상품 갯수가 100개인 Product를 생성한 후, 이를 1개씩 구매하는 과정을 동시에 100번 진행

- 락이 없는 경우 :  성공 < 100 , 실패 > 0 , 상품의 잔여 개수 > 0
- 락이 있는 경우 :  성공 == 100 , 실패 == 0 , 상품의 잔여 개수 == 0

락을 적용했기 때문에 실행시간은 늘어났으나, 모두 성공적으로 처리가 가능하게 됨

![concurrencyTest.png](images/concurrencyTest.png)
